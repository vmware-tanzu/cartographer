...
func (mapper *Mapper) ServiceAccountToWorkloadRequests(serviceAccountObject client.Object) []reconcile.Request {
	...
	var requests []reconcile.Request
	for _, workload := range list.Items {
		if mapper.serviceAccountWorkloadMatch(&workload, serviceAccountObject) {
			requests = append(requests, reconcile.Request{
				NamespacedName: types.NamespacedName{
					Name:      workload.Name,
					Namespace: workload.Namespace,
...
func (mapper *Mapper) serviceAccountWorkloadMatch(workload *v1alpha1.Workload, object client.Object) bool {
