...
var _ = Describe("Stamper", func() {
	Describe("Stamp", func() {
		...
		DescribeTable("tag evaluation of template",
			...
			Entry(`JSONPath error contains path into template and the expression that failed`,
				`$(params.sub.nonexistent)$`, `"5"`, "", "failed to interpolate template at path [key]: evaluate tag $(params.sub.nonexistent)$:"),
