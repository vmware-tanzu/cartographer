	...
	Context("Workloads", func() {
		Context("there are no previous resources", func() {
			...
			It("realizes each resource in supply chain order, accumulating output for each subsequent resource", func() {
				realizedResources, err := rlzr.Realize(context.TODO(), resourceRealizer, supplyChain.Name, realizer.MakeSupplychainOwnerResources(supplyChain), nil)
		...
		Context("there are previous resources", func() {
			...
			It("realizes each resource and does not update last transition time since the resource has not changed", func() {
				...
				realizedResources, err := rlzr.Realize(context.TODO(), resourceRealizer, supplyChain.Name, realizer.MakeSupplychainOwnerResources(supplyChain), previousResources)
				Expect(err).ToNot(HaveOccurred())
	...
	Context("Delivery", func() {
		Context("there are no previous resources", func() {
			...
			BeforeEach(func() {
				...
				resourceRealizer.DoCalls(func(ctx context.Context, resource realizer.OwnerResource, deliveryName string, outputs realizer.Outputs) (templates.Template, *unstructured.Unstructured, *templates.Output, error) {
					executedResourceOrder = append(executedResourceOrder, resource.Name)
			...
			It("realizes each resource in delivery order, accumulating output for each subsequent resource", func() {
				realizedResources, err := rlzr.Realize(ctx, resourceRealizer, delivery.Name, realizer.MakeDeliveryOwnerResources(delivery), nil)
		...
		Context("there are previous resources", func() {
			...
			It("realizes each resource and does not update last transition time since the resource has not changed", func() {
				...
				realizedResources, err := rlzr.Realize(context.TODO(), resourceRealizer, delivery.Name, realizer.MakeDeliveryOwnerResources(delivery), previousResources)
				Expect(err).ToNot(HaveOccurred())
